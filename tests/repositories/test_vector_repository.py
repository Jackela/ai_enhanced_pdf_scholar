...
from src.database.models import VectorIndexModel, DocumentModel
...
class TestVectorIndexRepository:
    ...
    def _create_test_document(self, **kwargs) -> int:
        ...
        result = self.db.execute(
            ...
            (defaults["title"], defaults["file_path"], defaults["file_hash"], 
             defaults["content_hash"], defaults["file_size"], defaults["page_count"])
        )
        return self.db.get_last_insert_id()
    
    def _create_test_vector_index(self, document_id: int = None, **kwargs) -> VectorIndexModel:
    ...
    @patch.object(VectorIndexModel, 'is_index_available')
    @patch('shutil.rmtree')
    def test_cleanup_invalid_indexes_with_file_removal(self, mock_rmtree, mock_is_available):
