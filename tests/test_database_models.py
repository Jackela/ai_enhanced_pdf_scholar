...
from src.database.models import DocumentModel, TagModel, VectorIndexModel
...
class TestDocumentModel:
    ...
    def test_document_model_allows_empty_title(self):
        ...
        doc = DocumentModel(
            title="", file_path=self.test_path, file_hash="abc123", file_size=1024
        )
        assert doc.title == ""
    ...
    def test_document_model_validation_empty_hash(self):
        ...
        with pytest.raises(ValueError, match="File hash cannot be empty"):
            DocumentModel(
                title="Test", file_path=self.test_path, file_hash="", file_size=1024
            )
    ...
    def test_document_model_validation_negative_size(self):
        ...
        with pytest.raises(ValueError, match="File size cannot be negative"):
            DocumentModel(
                title="Test", file_path=self.test_path, file_hash="abc123", file_size=-1
            )
    ...
    def test_from_file_factory_method(self):
        ...
        doc = DocumentModel.from_file(
            file_path=self.test_path, file_hash="test_hash", title="Custom Title"
        )
    ...
    def test_from_file_factory_auto_title(self):
        ...
        doc = DocumentModel.from_file(file_path=self.test_path, file_hash="test_hash")
    ...
    def test_from_file_factory_nonexistent_file(self):
        ...
        with pytest.raises(FileNotFoundError):
            DocumentModel.from_file(
                file_path="/nonexistent/file.pdf", file_hash="test_hash"
            )
    ...
    def test_update_access_time(self):
        ...
        doc = DocumentModel(
            title="Test", file_path=self.test_path, file_hash="hash", file_size=1024
        )
        assert doc.last_accessed is None
        # Update access time
    ...
    def test_get_display_name(self):
        ...
        doc = DocumentModel(
            title="", file_path=self.test_path, file_hash="hash", file_size=1024
        )
...
class TestVectorIndexModel:
    ...
    def test_vector_index_creation_minimal(self):
        ...
        index = VectorIndexModel(
            document_id=1, index_path="/path/to/index", index_hash="index_hash_123"
        )
    ...
    def test_vector_index_validation_document_id(self):
        ...
        with pytest.raises(ValueError, match="Document ID must be positive"):
            VectorIndexModel(document_id=-1, index_path="/path", index_hash="hash")
    ...
    def test_is_index_available_true(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            index = VectorIndexModel(
                document_id=1, index_path=str(index_path), index_hash="hash"
            )
    ...
    def test_is_index_available_false(self):
        ...
        # Non-existent path
        index = VectorIndexModel(
            document_id=1, index_path="/nonexistent/path", index_hash="hash"
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            index = VectorIndexModel(
                document_id=1, index_path=temp_dir, index_hash="hash"
            )
            assert index.is_index_available() is False
...
class TestTagModel:
    ...
    def test_tag_model_creation_complete(self):
        ...
        tag = TagModel(id=1, name="Research", color="#ff0000")
        assert tag.id == 1
    ...
    def test_from_database_row_factory(self):
        ...
        row = {"id": 1, "name": "database_tag", "color": "#00ff00"}
