...
class TestVectorIndexManager:
    ...
    def setup_method(self):
        ...
        self.manager = VectorIndexManager(
            ...
            storage_base_dir=self.temp_storage_dir
        )
    ...
    def test_create_index_storage_duplicate_path_handling(self):
        ...
        with patch('src.services.vector_index_manager.datetime') as mock_datetime:
            ...
            conflicting_path = self.manager.active_dir / f"doc_{document_id}_{index_hash[:8]}_20230101_120000"
    ...
    def test_create_index_storage_database_error(self):
        ...
        with patch.object(self.manager.vector_repo, 'create', side_effect=Exception("Database error")):
            with pytest.raises(VectorIndexManagerError, match="Index storage creation failed"):
    ...
    def test_move_index_to_storage_success(self):
        ...
        try:
            ...
            # Move files
            with patch.object(self.manager, 'verify_index_integrity', return_value=True):
    ...
    def test_move_index_integrity_verification_failure(self):
        ...
        try:
            ...
            with patch.object(self.manager, 'verify_index_integrity', return_value=False):
                with pytest.raises(VectorIndexManagerError, match="Index integrity verification failed"):
                    self.manager.move_index_to_storage(vector_index, source_dir)
    ...
    def test_verify_index_integrity_missing_directory(self):
        ...
        assert result["is_valid"] is False
        assert any("Index directory does not exist" in error for error in result["errors"])
    ...
    def test_verify_index_integrity_metadata_inconsistency(self):
        ...
        assert any("Metadata document_id mismatch" in warning for warning in result["warnings"])
    ...
    def test_remove_corrupted_indexes(self, mock_backup, mock_verify):
        ...
        corrupted_index = self.manager.create_index_storage(doc2_id, "corrupted_hash", 5)
    ...
    def test_verify_index_integrity_exception_handling(self):
        ...
        with patch.object(self.manager.vector_repo, 'find_by_id', side_effect=Exception("Database error")):
    ...
    def test_get_storage_statistics_exception_handling(self):
        ...
        with patch.object(self.manager.vector_repo, 'count', side_effect=Exception("Database error")):
            stats = self.manager.get_storage_statistics()
    ...
    def test_optimize_storage_exception_handling(self):
        ...
        with patch.object(self.manager, '_remove_orphaned_storage', side_effect=Exception("Cleanup error")):
