...
class TestEnhancedRAGService:
    ...
    def test_initialization_production_mode(self):
        ...
        with patch('src.services.enhanced_rag_service.EnhancedRAGService._initialize_llama_index'):
    ...
    def test_initialize_llama_index_success(self, mock_embedding, mock_gemini, mock_settings):
    ...
    def test_initialize_llama_index_import_error(self):
        ...
        with patch('src.services.enhanced_rag_service.Settings', side_effect=ImportError("No module")):
            with pytest.raises(RAGServiceError, match="LlamaIndex dependencies not available"):
                EnhancedRAGService(
                    api_key="test_key",
                    db_connection=self.db,
                    test_mode=False
    ...
    def test_initialize_llama_index_configuration_error(self):
        ...
        with patch('src.services.enhanced_rag_service.Settings', side_effect=Exception("Config error")):
            with pytest.raises(RAGServiceError, match="LlamaIndex initialization failed"):
                EnhancedRAGService(
                    api_key="test_key",
                    db_connection=self.db,
                    test_mode=False
    ...
    def test_build_index_from_pdf_production_mode(self, mock_reader, mock_index):
        ...
        mock_reader.return_value.load_data.return_value = [MagicMock()]  # Mock documents
    ...
    def test_build_index_from_document_existing_index_with_overwrite(self):
        ...
        with patch.object(self.service, '_get_chunk_count', return_value=10):
            vector_index2 = self.service.build_index_from_document(created_doc, overwrite=True)
    ...
    @patch('src.services.enhanced_rag_service.EnhancedRAGService._initialize_llama_index')
    ...
    def test_query_document_success(self):
        ...
        response = self.service.query_document("What is this document about?", created_doc.id)
    ...
    def test_get_cache_info_exception(self):
        ...
        with patch.object(self.service.cache_dir, 'exists', side_effect=Exception("FS error")):
            info = self.service.get_cache_info()
    ...
    def test_get_enhanced_cache_info_exception(self):
        ...
        with patch.object(self.service.vector_repo, 'get_index_statistics', side_effect=Exception("DB error")):
            info = self.service.get_enhanced_cache_info()
            assert "error" in info
    ...
    def test_get_document_index_status_exception(self):
        ...
        with patch.object(self.service.vector_repo, 'find_by_document_id', side_effect=Exception("DB error")):
    ...
    def test_cleanup_orphaned_indexes(self):
        ...
        with patch.object(self.service.vector_repo, 'cleanup_orphaned_indexes', return_value=3):
            removed_count = self.service.cleanup_orphaned_indexes()
            assert removed_count == 3
    
    def test_cleanup_orphaned_indexes_exception(self):
        """Test cleanup with exception."""
        with patch.object(self.service.vector_repo, 'cleanup_orphaned_indexes', side_effect=Exception("Cleanup failed")):
