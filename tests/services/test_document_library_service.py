...
class TestDocumentLibraryService:
    ...
    @patch('src.services.content_hash_service.ContentHashService.calculate_combined_hashes')
    ...
    def test_import_document_copy_failure(self, mock_copy):
        ...
        with pytest.raises(DocumentImportError, match="Failed to copy file to managed storage"):
            self.service.import_document(pdf_path)
    ...
    def test_get_documents_with_limit(self):
        ...
        for i in range(5):
            pdf_path = self.get_fixture_path("simple_text.pdf")
            self.service.import_document(pdf_path, title=f"Document {i}", check_duplicates=False)
    ...
    def test_get_recent_documents(self):
        ...
        doc1 = self.service.import_document(pdf_path, title="Older Document", check_duplicates=False)
        ...
        doc2 = self.service.import_document(pdf_path, title="Newer Document", check_duplicates=False)
    ...
    def test_delete_document_with_vector_index(self):
        ...
        self.db.execute(
            ...
            (document.id, vector_index.index_path, vector_index.chunk_count, vector_index.index_hash)
        )
    ...
    def test_find_duplicate_documents_by_content(self):
        ...
        doc1 = self.service.import_document(pdf1, title="Document 1", check_duplicates=False)
        doc2 = self.service.import_document(pdf2, title="Document 2", check_duplicates=False)
    ...
    def test_verify_document_integrity_hash_mismatch(self, mock_hash):
        ...
        assert any("File hash mismatch" in error for error in integrity_result["errors"])
    ...
    def test_advanced_search_by_title(self):
        ...
        pdf_path = self.get_fixture_path("simple_text.pdf")
        doc1 = self.service.import_document(pdf_path, title="Python Programming", check_duplicates=False)
        doc2 = self.service.import_document(pdf_path, title="Java Programming", check_duplicates=False)
        doc3 = self.service.import_document(pdf_path, title="Data Science", check_duplicates=False)
    ...
    def test_advanced_search_by_date_range(self):
        ...
        doc1 = self.service.import_document(pdf_path, title="Old Document", check_duplicates=False)
        ...
        doc2 = self.service.import_document(pdf_path, title="New Document", check_duplicates=False)
    ...
    def test_advanced_search_multiple_criteria(self):
        ...
        pdf_path = self.get_fixture_path("simple_text.pdf")
        doc1 = self.service.import_document(pdf_path, title="Python Guide", check_duplicates=False)
        doc2 = self.service.import_document(pdf_path, title="Python Advanced", check_duplicates=False)
        doc3 = self.service.import_document(pdf_path, title="Java Guide", check_duplicates=False)
