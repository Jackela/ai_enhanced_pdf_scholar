...
class TestRAGCacheService:
    ...
    def _insert_cache_entry(self, query: str, document_id: int, response: str, 
                           created_at: datetime = None, accessed_at: datetime = None) -> int:
        ...
        result = self.db.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            query_hash, query, document_id, response,
            created_at.isoformat(), accessed_at.isoformat(),
            len(query), len(response)
        ))
        return self.db.get_last_insert_id()
    ...
    def test_initialization_table_creation_error(self):
        ...
        with patch.object(self.db, 'execute', side_effect=Exception("Table creation failed")):
            with pytest.raises(RAGCacheServiceError, match="Cache initialization failed"):
    ...
    def test_cache_response_empty_query(self):
        ...
        success = self.cache_service.cache_response("", self.test_doc_id, "Some response")
    ...
    def test_cache_response_duplicate_skip(self):
        ...
        count = self.db.fetch_one("SELECT COUNT(*) as count FROM rag_query_cache")["count"]
        assert count == 1
    
    def test_cache_response_different_documents(self):
        """Test caching same query for different documents."""
        doc2_id = self._create_test_document(title="Doc 2", file_path="/doc2.pdf", file_hash="def789")
    ...
    def test_get_cached_response_exact_match(self):
        ...
        cached_response = self.cache_service.get_cached_response(query, self.test_doc_id)
    ...
    def test_get_cached_response_semantic_similarity(self):
        ...
        cached_response = self.cache_service.get_cached_response(similar_query, self.test_doc_id)
    ...
    def test_get_cached_response_similarity_threshold(self):
        ...
        cached_response = self.cache_service.get_cached_response(dissimilar_query, self.test_doc_id)
    ...
    def test_get_cached_response_database_error(self):
        ...
        with patch.object(self.db, 'fetch_one', side_effect=Exception("Database error")):
            cached_response = self.cache_service.get_cached_response("query", self.test_doc_id)
            assert cached_response is None
    ...
    def test_clear_cache_success(self):
        ...
        for i in range(5):
            self.cache_service.cache_response(f"Query {i}", self.test_doc_id, f"Response {i}")
    ...
    def test_cache_ttl_expiration(self):
        ...
        with patch('src.services.rag_cache_service.datetime') as mock_datetime:
            ...
            expired_response = short_ttl_cache.get_cached_response(query, self.test_doc_id)
    ...
    def test_cache_size_enforcement_lru(self):
        ...
        remaining_entries = self.db.fetch_all("SELECT query_text FROM rag_query_cache ORDER BY accessed_at")
    ...
    def test_cache_optimization(self):
        ...
        self._insert_cache_entry("Old query", self.test_doc_id, "Old response", 
                                created_at=old_time, accessed_at=old_time)
        
        # Insert valid entry
        self._insert_cache_entry("Recent query", self.test_doc_id, "Recent response", 
                                created_at=recent_time, accessed_at=recent_time)
        
        # Insert duplicate query (different hash)
        self._insert_cache_entry("Recent query", self.test_doc_id, "Duplicate response", 
                                created_at=recent_time, accessed_at=recent_time)
    ...
    def test_get_cache_statistics_with_data(self):
        ...
        doc_counts = {item["document_id"]: item["count"] for item in stats["document_distribution"]}
    ...
    def test_find_similar_query_jaccard_similarity(self):
        ...
        for similar_query in similar_queries:
            similar_entry = self.cache_service._find_similar_query(similar_query, self.test_doc_id)
    ...
    def test_find_similar_query_no_match(self):
        ...
        self.cache_service.cache_response("weather forecast", self.test_doc_id, "Sunny today")
    ...
    def test_find_similar_query_database_error(self):
        ...
        with patch.object(self.db, 'fetch_all', side_effect=Exception("Database error")):
            similar_entry = self.cache_service._find_similar_query("query", self.test_doc_id)
            assert similar_entry is None
    ...
    def test_clean_expired_entries(self):
        ...
        self._insert_cache_entry("Expired query", self.test_doc_id, "Expired response", 
                                created_at=expired_time)
        self._insert_cache_entry("Valid query", self.test_doc_id, "Valid response", 
    ...
    def test_remove_duplicate_queries(self):
        ...
        self._insert_cache_entry(query, self.test_doc_id, "First response", 
                                created_at=now - timedelta(minutes=10))
        self._insert_cache_entry(query, self.test_doc_id, "Second response", 
                                created_at=now - timedelta(minutes=5))
        self._insert_cache_entry(query, self.test_doc_id, "Latest response", 
                                created_at=now)
    ...
    def test_update_access(self):
        ...
        cache_id = self._insert_cache_entry("Test query", self.test_doc_id, "Test response")
        
        # Get original access info
        original_entry = self.db.fetch_one("SELECT * FROM rag_query_cache WHERE id = ?", (cache_id,))
        original_access_time = original_entry["accessed_at"]
        original_access_count = original_entry["access_count"]
        ...
        # Verify updates
        updated_entry = self.db.fetch_one("SELECT * FROM rag_query_cache WHERE id = ?", (cache_id,))
    ...
    def test_concurrent_operations_simulation(self):
        ...
        doc2_id = self._create_test_document(title="Doc 2", file_path="/doc2.pdf", file_hash="def789")
        doc3_id = self._create_test_document(title="Doc 3", file_path="/doc3.pdf", file_hash="ghi101")
        ...
        test_data = [
            ...
            (self.test_doc_id, "Another query about doc 1", "Another response for doc 1")
        ]
        ...
        assert self.cache_service.get_cached_response("Query about doc 1", self.test_doc_id) is not None
        assert self.cache_service.get_cached_response("Query about doc 2", doc2_id) is None
        assert self.cache_service.get_cached_response("Query about doc 3", doc3_id) is not None
