...
class TestDatabaseConnectionOptimized:
    ...
    def test_fetch_all_method(self, clean_db_connection):
        ...
        db.execute_many("INSERT INTO test_fetch (value) VALUES (?)", [(i,) for i in range(5)])
    ...
    def test_transaction_rollback(self, clean_db_connection):
        ...
        # Create test table
        db.execute("CREATE TABLE test_rollback (id INTEGER PRIMARY KEY, name TEXT UNIQUE)")
    ...
    def test_thread_safety(self, clean_db_connection, thread_test_helper):
        ...
        # Create test table
        db.execute("CREATE TABLE test_threads (id INTEGER PRIMARY KEY, thread_id INTEGER, iteration INTEGER)")
        
        def worker_func(thread_id, iteration):
            """Worker function for thread safety test."""
            db.execute("INSERT INTO test_threads (thread_id, iteration) VALUES (?, ?)", 
                      (thread_id, iteration))
            time.sleep(0.001)  # Small delay to encourage race conditions
        
        # Run concurrent test
        results, errors = thread_test_helper.run_concurrent_test(worker_func, thread_count=3, iterations=5)
    ...
    def test_row_factory_dict_access(self, clean_db_connection):
        ...
        # Create and populate test table
        db.execute("CREATE TABLE test_dict (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)")
...
class TestDatabasePerformance:
    ...
    def test_query_performance(self, clean_db_connection):
        ...
        assert duration < 0.5, f"Query performance took {duration:.2f}s, expected < 0.5s"
