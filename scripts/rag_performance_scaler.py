...
class RAGWorkloadAnalyzer:
    ...
    async def analyze_query_complexity(self, metrics: dict[str, float]) -> float:
        ...
        search_results_factor = min(
            1.0, metrics.get("vector_search_results_count", 0) / 50.0
        )
        ...
        index_factor = min(
            1.0, metrics.get("vector_index_size_mb", 0) / 1000.0
        )  # Normalize to 1GB
    ...
    async def create_workload_profile(
        ...
        query_rate = max(
            0.01, metrics.get("rag_query_rate", 0.01)
        ...
        return RAGWorkloadProfile(
            ...
            processing_queue_depth=int(
                metrics.get("document_processing_queue_depth", 0)
            ),
...
class RAGPerformanceScaler:
    ...
    async def calculate_rag_scaling_factors(
        ...
        if (
            profile.p95_query_time_ms
            > self.scaling_thresholds["max_p95_response_time_ms"]
        ):
            factors["response_time"] = 2.0  # Aggressive scale-up
        elif (
            profile.p95_query_time_ms
            > self.scaling_thresholds["target_p95_response_time_ms"]
        ):
            ...
            factors["response_time"] = 1.0 + (
                (profile.p95_query_time_ms - target) / (max_time - target)
            )
        else:
        ...
        max_queries_per_replica = self.scaling_thresholds[
            "max_concurrent_queries_per_replica"
        ]
        ...
        if (
            profile.memory_per_query_mb
            > self.scaling_thresholds["max_memory_per_query_mb"]
        ):
        ...
        # Queue depth factor
        if (
            profile.processing_queue_depth
            > self.scaling_thresholds["max_processing_queue_depth"]
        ):
            queue_factor = (
                profile.processing_queue_depth
                / self.scaling_thresholds["target_processing_queue_depth"]
            )
        ...
        if profile.query_complexity_score > 0.7:
            factors["complexity"] = (
                1.0 + (profile.query_complexity_score - 0.7) * 2
            )  # Up to 1.6x
        else:
        ...
        if profile.cache_hit_rate < self.scaling_thresholds["min_cache_hit_rate"]:
            cache_penalty = (
                self.scaling_thresholds["min_cache_hit_rate"] - profile.cache_hit_rate
            ) * 2
        ...
        if (
            profile.answer_quality_score
            < self.scaling_thresholds["min_answer_quality_score"]
        ):
    ...
    async def calculate_optimal_replicas(
        self, profile: RAGWorkloadProfile, current_replicas: int
    ) -> tuple[int, float]:
        ...
        scaling_multiplier = sum(
            factors.get(factor, 1.0) * weight for factor, weight in weights.items()
        )
        ...
        if len(self.scaling_history) >= 3:
            recent_accuracy = np.mean(
                [h.get("accuracy", 0.8) for h in self.scaling_history[-3:]]
            )
    ...
    async def estimate_performance_impact(
        self, profile: RAGWorkloadProfile, current_replicas: int, new_replicas: int
    ) -> tuple[float, float]:
        ...
        if new_replicas <= current_replicas:
            ...
            expected_time_increase = (
                (replica_ratio - 1) * profile.p95_query_time_ms * 0.3
            )  # 30% impact
            expected_query_time_improvement = (
                -expected_time_increase
            )  # Negative = worse
            ...
            expected_query_time_improvement = (
                profile.p95_query_time_ms * (1 - load_reduction_factor) * 0.7
            )

            # Quality might improve due to less resource contention
            expected_quality_impact = min(
                0.1, (replica_ratio - 1) * 0.05
            )  # Up to +10% quality

        return expected_query_time_improvement, expected_quality_impact

    async def calculate_cost_impact(
        self, current_replicas: int, new_replicas: int, profile: RAGWorkloadProfile
    ) -> float:
        ...
        memory_gb_per_replica = (
            profile.memory_per_query_mb * 10 / 1024
        )  # Estimate based on query memory

        # Calculate cost change per hour
        cpu_cost_change = (
            replica_change
            * cpu_cores_per_replica
            * self.cost_factors["cpu_cost_per_core_hour"]
        )
        memory_cost_change = (
            replica_change
            * memory_gb_per_replica
            * self.cost_factors["memory_cost_per_gb_hour"]
        )
    ...
    async def make_scaling_decision(self, current_replicas: int) -> RAGScalingDecision:
        ...
        try:
            ...
            optimal_replicas, confidence = await self.calculate_optimal_replicas(
                profile, current_replicas
            )

            # Get scaling factors for explanation
            scaling_factors = await self.calculate_rag_scaling_factors(
                profile, current_replicas
            )
            ...
            cost_impact = await self.calculate_cost_impact(
                current_replicas, optimal_replicas, profile
            )
            ...
            if optimal_replicas > current_replicas:
                reasoning_parts.append(
                    f"Scale up from {current_replicas} to {optimal_replicas} replicas"
                )
                if (
                    profile.p95_query_time_ms
                    > self.scaling_thresholds["target_p95_response_time_ms"]
                    ...
                    reasoning_parts.append(
                        f"P95 query time {profile.p95_query_time_ms:.0f}ms exceeds target"
                    )
                if (
                    profile.concurrent_queries / current_replicas
                    > self.scaling_thresholds["max_concurrent_queries_per_replica"]
                ):
                ...
                if (
                    ...
                    reasoning_parts.append(
                        f"Processing queue depth at {profile.processing_queue_depth}"
                    )

            elif optimal_replicas < current_replicas:
                reasoning_parts.append(
                    f"Scale down from {current_replicas} to {optimal_replicas} replicas"
                )
                reasoning_parts.append(
                    "Current utilization allows for cost optimization"
                )
                if profile.cache_hit_rate > 0.8:
                    reasoning_parts.append(
                        f"High cache hit rate ({profile.cache_hit_rate:.1%}) reduces load"
                    )
            ...
            if (
                ...
                reasoning_parts.append(
                    f"Quality score {profile.answer_quality_score:.2f} below threshold"
                )

            if profile.query_complexity_score > 0.7:
                reasoning_parts.append(
                    f"High query complexity ({profile.query_complexity_score:.2f}) detected"
                )
    ...
    async def get_current_replicas(self) -> int:
        ...
        try:
            deployment = self.apps_v1.read_namespaced_deployment(
                name="ai-pdf-scholar-backend", namespace="ai-pdf-scholar"
            )
    ...
    async def run_continuous(self, interval_seconds: int = 120) -> None:
        ...
        while True:
            try:
                ...
                logger.info(
                    f"    Query Complexity: {decision.workload_profile.query_complexity_score:.2f}"
                )
                logger.info(
                    f"    P95 Query Time: {decision.workload_profile.p95_query_time_ms:.0f}ms"
                )
                logger.info(
                    f"    Cache Hit Rate: {decision.workload_profile.cache_hit_rate:.1%}"
                )
                logger.info(
                    f"    Quality Score: {decision.workload_profile.answer_quality_score:.2f}"
                )
                logger.info(
                    f"    Queue Depth: {decision.workload_profile.processing_queue_depth}"
                )
...
async def main() -> None:
    ...
    parser.add_argument(
        "--mode",
        choices=["analyze", "decide", "continuous"],
        default="continuous",
        help="Operation mode",
    )
    parser.add_argument(
        "--interval", type=int, default=120, help="Scaling check interval in seconds"
    )
    ...
    if args.mode == "analyze":
        ...
        profile = await orchestrator.scaler.workload_analyzer.create_workload_profile(
            metrics
        )
