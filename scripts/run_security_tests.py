...
class SecurityTestOrchestrator:
    ...
    def _generate_combined_analysis(
        ...
        pentest_findings = pentest_results.get('findings_summary', {}).get('all_findings', [])
        ...
        severity_counts = {
            'critical': len([v for v in unique_vulnerabilities if v.get('severity') == 'critical']),
            'high': len([v for v in unique_vulnerabilities if v.get('severity') == 'high']),
            'medium': len([v for v in unique_vulnerabilities if v.get('severity') == 'medium']),
            'low': len([v for v in unique_vulnerabilities if v.get('severity') == 'low'])
        }
        ...
        compliance_assessment = self._generate_compliance_assessment(unique_vulnerabilities)
    ...
    def _determine_security_posture(self, severity_counts: dict[str, int], risk_score: int) -> dict[str, Any]:
        ...
        if severity_counts['critical'] > 0:
            ...
            level = 'High Risk'
            description = 'High priority remediation needed. Multiple high-severity issues.'
            color = 'orange'
...
web application security, and common penetration testing techniques.
    ...
    def _generate_compliance_assessment(self, vulnerabilities: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        owasp_issues = len([v for v in vulnerabilities if any(
            owasp_cat in v.get('type', '') for owasp_cat in [
        ...
        return {
            ...
            'pci_dss': {
                ...
                'status': 'Ready' if critical_count == 0 and high_count <= 2 else 'Not Ready'
            },
            'iso_27001': {
                'aligned': critical_count == 0 and high_count <= 3,
                'major_gaps': critical_count + high_count,
                'status': 'Aligned' if critical_count == 0 and high_count <= 3 else 'Gaps Identified'
            },
            'nist_cybersecurity_framework': {
                'mature': critical_count == 0 and high_count == 0,
                'status': 'Mature' if critical_count == 0 and high_count == 0 else 'Developing'
            }
        }
                ...
                if vuln_severity == severity:
                    ...
                    action_item = {
                        ...
                        'affected_components': list(set([
                            vuln.get('endpoint', vuln.get('component', 'Unknown'))
                            for vuln in vulns
                        ]))
                    }
    ...
    def _get_action_description(self, vuln_type: str) -> str:
        ...
        return actions.get(vuln_type, 'Review and remediate according to best practices')
    ...
    async def _save_assessment_results(self, results: dict[str, Any]) -> None:
        ...
        if 'combined_analysis' in results and 'executive_summary' in results['combined_analysis']:
    ...
    async def _save_vulnerabilities_csv(self, results: dict[str, Any]) -> None:
        ...
        if 'combined_analysis' in results and 'all_vulnerabilities' in results['combined_analysis']:
            vulnerabilities = results['combined_analysis']['all_vulnerabilities']

            with open(csv_file, 'w', newline='', encoding='utf-8') as f:
                if vulnerabilities:
                    fieldnames = ['type', 'severity', 'description', 'endpoint', 'recommendation']
                    ...
                    for vuln in vulnerabilities:
                        writer.writerow({
                            ...
                            'endpoint': vuln.get('endpoint', vuln.get('component', '')),
...
async def main():
    ...
    parser.add_argument('--output', '-o', default='security_reports', help='Output directory for reports')
    parser.add_argument('--tests', '-T', choices=['owasp', 'pentest', 'all'], default='all',
    ...
    try:
        ...
        if 'combined_analysis' in results:
            ...
            print(f"Total Vulnerabilities: {analysis.get('total_unique_vulnerabilities', 0)}")
