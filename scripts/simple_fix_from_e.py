...
def fix_file(filepath: Path) -> int:
    ...
    try:
        ...
        content = re.sub(
            r'(error_type="[^"]*")\s+from\s+(\w+)\)', r"\1) from \2", content
        )

        # Fix patterns where 'from e' is inside the detail string
        content = re.sub(r'(detail="[^"]*)\s+from\s+(\w+)', r'\1") from \2', content)

        # Fix patterns where 'from e' is inside a string literal
        content = re.sub(r'(detail=f?"[^"]*)\s+from\s+(\w+)', r'\1") from \2', content)

        # Fix patterns with detail=str(e from e)
        content = re.sub(
            r"detail=str\(e\s+from\s+e\)", r"detail=str(e)) from e", content
        )
