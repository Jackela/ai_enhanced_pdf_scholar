...
class CIPerformanceOptimizer:
    ...
    def _collect_performance_metrics(self) -> dict[str, Any]:
        ...
        performance = {
            "pipeline_stages": {
                "change_detection": self._measure_stage_performance(
                    "change-detection", 45
                ),
                "quality_matrix": self._measure_stage_performance(
                    "quality-matrix", 180
                ),
                "build_matrix": self._measure_stage_performance("build-matrix", 240),
                "test_matrix": self._measure_stage_performance("test-matrix", 300),
                "performance_benchmarks": self._measure_stage_performance(
                    "benchmarks", 120
                ),
            },
        ...
        total_time = sum(
            ...
            for stage in performance["pipeline_stages"].values()
        )
    ...
    def _measure_stage_performance(
        self, stage_name: str, baseline_seconds: int
    ) -> dict[str, Any]:
    ...
    def print_summary(self, report: dict[str, Any]) -> None:
        ...
        print(
            f"📈 Overall Performance Score: {summary['overall_performance_score']:.1%}"
        )
        ...
        print("\n💾 Cache Performance:")
        print(
            f"   Test Results: {cache['test_results_cache']['hit_rate']:.1%} hit rate"
        )
        print(
            f"   Build Artifacts: {cache['build_artifacts_cache']['hit_rate']:.1%} hit rate"
        )
        print(
            f"   Dependencies: {cache['dependencies_cache']['hit_rate']:.1%} hit rate"
        )
