...
class PerformanceBaselineEstablisher:
    ...
    def _benchmark_database_operations(self) -> dict[str, Any]:
        ...
        try:
            ...
            db_metrics["select_operations"] = {
                ...
                "meets_target": self._percentile(select_times, 95)
                < self.targets["db_query_95th_percentile_ms"],
            }

            print(
                f"   - SELECT operations: {db_metrics['select_operations']['avg_ms']:.2f} ms avg, "
            ...
            db_metrics["insert_operations"] = {
                ...
                "meets_target": self._percentile(insert_times, 95)
                < self.targets["db_query_95th_percentile_ms"],
            }

            print(
                f"   - INSERT operations: {db_metrics['insert_operations']['avg_ms']:.2f} ms avg, "
            ...
            db_metrics["complex_queries"] = {
                ...
                "meets_target": self._percentile(complex_times, 95)
                < self.targets["db_query_95th_percentile_ms"],
            }

            print(
                f"   - Complex queries: {db_metrics['complex_queries']['avg_ms']:.2f} ms avg, "
            ...
            with self.db.get_connection() as conn:
                conn.execute(
                    "DELETE FROM documents WHERE title LIKE 'Performance Test Document%'"
                )
                conn.commit()
        ...
        try:
            ...
            for _ in range(30):
                ...
                try:
                    response = requests.get(
                        "http://127.0.0.1:8001/api/system/health", timeout=5
                    )
                    end = time.perf_counter()
            ...
            if health_times:
                api_metrics["health_endpoint"] = {
                    ...
                    "meets_target": self._percentile(health_times, 95)
                    < self.targets["api_response_95th_percentile_ms"],
                }
                print(
                    f"   - /health endpoint: {api_metrics['health_endpoint']['avg_ms']:.0f} ms avg"
                )
            ...
            for _ in range(20):
                ...
                try:
                    response = requests.get(
                        "http://127.0.0.1:8001/api/documents/", timeout=10
                    )
                    end = time.perf_counter()
                    if response.status_code == 200:
            ...
            if doc_times:
                api_metrics["documents_endpoint"] = {
                    ...
                    "meets_target": self._percentile(doc_times, 95)
                    < self.targets["api_response_95th_percentile_ms"],
                }
                print(
                    f"   - /api/documents: {api_metrics['documents_endpoint']['avg_ms']:.0f} ms avg"
                )
    ...
    def _benchmark_rag_processing(self) -> dict[str, Any]:
        ...
        try:
            ...
            rag_metrics["document_indexing"] = {
                ...
                "meets_target": statistics.mean(index_times)
                < self.targets["document_processing_s"],
            }

            print(
                f"   - Document indexing: {rag_metrics['document_indexing']['avg_s']:.2f} s per doc avg"
            )
            ...
            rag_metrics["query_processing"] = {
                ...
                "meets_target": self._percentile(query_times, 90)
                < self.targets["rag_query_90th_percentile_s"],
            }

            print(
                f"   - Query processing: {rag_metrics['query_processing']['avg_s']:.2f} s avg, "
    ...
    def _analyze_memory_usage(self) -> dict[str, Any]:
        ...
        memory_metrics = {
            ...
            "efficiency_mb_per_operation": (max(memory_readings) - baseline_memory)
            / 20,
        ...
        print(
            f"   - Memory efficiency: {memory_metrics['efficiency_mb_per_operation']:.2f} MB/operation"
        )
    ...
    def _benchmark_system_resources(self) -> dict[str, Any]:
        ...
        resource_metrics = {
            ...
            "disk_write_throughput_mb_s": (len(test_data.encode()) / 1024 / 1024)
            / (time.perf_counter() - disk_start),
            "disk_read_throughput_mb_s": (len(content.encode()) / 1024 / 1024)
            / read_time,
        ...
        print(
            f"   - Disk read throughput: {resource_metrics['disk_read_throughput_mb_s']:.1f} MB/s"
        )
        print(
            f"   - Available memory: {resource_metrics['available_memory_gb']:.1f} GB"
        )
    ...
    def _generate_baseline_report(self) -> None:
        ...
        if "error" not in db_perf:
            ...
            if "select_operations" in db_perf:
                ...
                status = (
                    "✅ PASS" if select_perf["meets_target"] else "❌ NEEDS IMPROVEMENT"
                )
                print(
                    f"  - SELECT: {select_perf['avg_ms']:.1f}ms avg, {select_perf['p95_ms']:.1f}ms (95%) {status}"
                )

            if "insert_operations" in db_perf:
                insert_perf = db_perf["insert_operations"]
                status = (
                    "✅ PASS" if insert_perf["meets_target"] else "❌ NEEDS IMPROVEMENT"
                )
                print(
                    f"  - INSERT: {insert_perf['avg_ms']:.1f}ms avg, {insert_perf['p95_ms']:.1f}ms (95%) {status}"
                )
            ...
            if "complex_queries" in db_perf:
                ...
                status = (
                    "✅ PASS"
                    if complex_perf["meets_target"]
                    else "❌ NEEDS IMPROVEMENT"
                )
                print(
                    f"  - COMPLEX: {complex_perf['avg_ms']:.1f}ms avg, {complex_perf['p95_ms']:.1f}ms (95%) {status}"
                )
        ...
        if "error" not in api_perf:
            ...
            if "health_endpoint" in api_perf:
                ...
                status = (
                    "✅ PASS" if health_perf["meets_target"] else "❌ NEEDS IMPROVEMENT"
                )
                print(
                    f"  - Health endpoint: {health_perf['avg_ms']:.0f}ms avg {status}"
                )

            if "documents_endpoint" in api_perf:
                doc_perf = api_perf["documents_endpoint"]
                status = (
                    "✅ PASS" if doc_perf["meets_target"] else "❌ NEEDS IMPROVEMENT"
                )
                print(
                    f"  - Documents endpoint: {doc_perf['avg_ms']:.0f}ms avg {status}"
                )
        ...
        if "error" not in rag_perf:
            ...
            if "document_indexing" in rag_perf:
                ...
                status = (
                    "✅ PASS" if index_perf["meets_target"] else "❌ NEEDS IMPROVEMENT"
                )
                print(
                    f"  - Document indexing: {index_perf['avg_s']:.2f}s per doc {status}"
                )

            if "query_processing" in rag_perf:
                query_perf = rag_perf["query_processing"]
                status = (
                    "✅ PASS" if query_perf["meets_target"] else "❌ NEEDS IMPROVEMENT"
                )
                print(
                    f"  - Query processing: {query_perf['avg_s']:.2f}s avg, {query_perf['p90_s']:.2f}s (90%) {status}"
                )
        ...
        if memory_perf:
            status = (
                "✅ PASS" if memory_perf["meets_target"] else "❌ NEEDS IMPROVEMENT"
            )
            print("\n📊 Memory Usage:")
            print(
                f"  - Base: {memory_perf['baseline_mb']:.0f}MB, Peak: {memory_perf['peak_mb']:.0f}MB {status}"
            )
            print(
                f"  - Memory efficiency: {memory_perf['efficiency_mb_per_operation']:.2f}MB/operation"
            )
        ...
        if total_tests > 0:
            ...
            print(
                f"  - Performance Tests: {passed_tests}/{total_tests} passed ({pass_rate:.1f}%)"
            )
    ...
    def _save_baselines(self) -> None:
        ...
        if "database_performance" in self.baselines:
            ...
            if "select_operations" in db:
                summary["key_metrics"]["db_select_p95_ms"] = db["select_operations"][
                    "p95_ms"
                ]
        ...
        if "api_performance" in self.baselines:
            ...
            if "health_endpoint" in api:
                summary["key_metrics"]["api_health_avg_ms"] = api["health_endpoint"][
                    "avg_ms"
                ]
