...
class JWTSecretsAdapter:
    ...
    def ensure_keys_exist(self) -> Tuple[bytes, bytes]:
        ...
        private_key = self.secrets_manager.get_secret("jwt_private_key", SecretType.JWT_PRIVATE_KEY)
        public_key = self.secrets_manager.get_secret("jwt_public_key", SecretType.JWT_PUBLIC_KEY)
...
def monkey_patch_jwt_config():
    ...
    OldJWTConfig.ensure_keys_exist = classmethod(lambda cls: adapter.ensure_keys_exist())
...
def check_and_rotate_secrets() -> Dict[str, bool]:
    ...
    for key, metadata in secrets_to_rotate:
        logger.info(f"Secret {key} needs rotation (last rotated: {metadata.last_rotated})")
...
def get_secret_health_status() -> Dict[str, Any]:
    ...
    health = {
        ...
        "configuration": {
            ...
            "fallback_providers": [p.value for p in secrets_manager.config.fallback_providers],
        },
        "secrets": {
            "total": len(secrets_manager.list_secrets()),
            "api_keys_configured": config.api_keys.model_dump(exclude_unset=True, exclude_none=True),
