...
class RedisClusterConfig:
    ...
    def validate(self, environment: Environment) -> list[str]:
        ...
        if environment.is_production() and self.replication_factor < 2:
            issues.append(
                "Production Redis cluster should have replication_factor >= 2"
            )
...
class L1CacheConfig:
    ...
    def validate(self, environment: Environment) -> list[str]:
        ...
        total_size = (
            self.hot_data_size_mb + self.warm_data_size_mb + self.cold_data_size_mb
        )
...
class L2CacheConfig:
    ...
    def validate(self, environment: Environment) -> list[str]:
        ...
        if self.max_ttl_seconds < self.default_ttl_seconds:
            issues.append(
                "L2 cache max_ttl_seconds cannot be less than default_ttl_seconds"
            )
...
class CacheCoherencyConfig:
    ...
    def validate(self, environment: Environment) -> list[str]:
        ...
        if self.invalidation_strategy not in valid_invalidation:
            issues.append(
                f"Invalid invalidation strategy: {self.invalidation_strategy}"
            )
...
class CachingConfig:
    ...
    require_ml_dependencies: bool = False
    ...
    def _load_redis_cluster_config(self, environment: Environment):
        ...
        if os.getenv("REDIS_CLUSTER_NODES"):
            ...
            if redis_url.startswith("redis://"):
                ...
                port = (
                    int(url_parts[1]) if len(url_parts) > 1 and url_parts[1] else 6379
                )
    ...
    def _load_l2_config(self, environment: Environment):
        ...
        self.l2_cache = L2CacheConfig(
            ...
            compression_enabled=os.getenv("L2_COMPRESSION_ENABLED", "true").lower()
            == "true",
            compression_threshold_bytes=int(
                os.getenv("L2_COMPRESSION_THRESHOLD", "1024")
            ),
            batch_size=int(os.getenv("L2_BATCH_SIZE", "100")),
            write_behind_enabled=os.getenv("L2_WRITE_BEHIND_ENABLED", "true").lower()
            == "true",
            write_behind_flush_interval=int(
                os.getenv("L2_WRITE_BEHIND_INTERVAL", "30")
            ),
        )
    ...
    def _load_coherency_config(self, environment: Environment):
        ...
        self.coherency = CacheCoherencyConfig(
            ...
            coherency_check_interval=int(
                os.getenv("CACHE_COHERENCY_CHECK_INTERVAL", "300")
            ),
            enable_versioning=os.getenv("CACHE_VERSIONING_ENABLED", "true").lower()
            == "true",
            enable_monitoring=os.getenv("CACHE_MONITORING_ENABLED", "true").lower()
            == "true",
        )
    ...
    def _load_global_config(self, environment: Environment):
        ...
        self.cache_key_prefix = os.getenv(
            "CACHE_KEY_PREFIX", f"pdf_scholar:{environment.value}:"
        )
        self.enable_performance_monitoring = (
            os.getenv("CACHE_PERFORMANCE_MONITORING", "true").lower() == "true"
        )
        self.metrics_collection_interval = int(
            os.getenv("CACHE_METRICS_INTERVAL", "60")
        )
        self.enable_ml_cache = (
            os.getenv("CACHE_ML_OPTIMIZATIONS_ENABLED", "true").lower() == "true"
        )
        self.require_ml_dependencies = (
            os.getenv("CACHE_ML_DEPS_REQUIRED", "false").lower() == "true"
        )

        # Cache warming settings
        self.enable_cache_warming = (
            os.getenv("CACHE_WARMING_ENABLED", "true").lower() == "true"
        )
        self.warming_batch_size = int(os.getenv("CACHE_WARMING_BATCH_SIZE", "50"))
        self.prefetch_popular_content = (
            os.getenv("CACHE_PREFETCH_POPULAR", "true").lower() == "true"
        )
    ...
    def validate(self, environment: Environment) -> list[str]:
        ...
        if self.require_ml_dependencies and not self.enable_ml_cache:
            all_issues.append(
                "ML dependencies cannot be required when ML cache is disabled"
            )

        # Performance validation
        if (
            self.l1_cache.enabled
            and self.l1_cache.max_size_mb > 1024
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "ml_cache": {
                "enabled": self.enable_ml_cache,
                "deps_required": self.require_ml_dependencies,
            },
            "redis_cluster": {
    ...
    if issues:
        ...
        if critical_issues and environment.requires_strict_security():
            raise ConfigValidationError(
                "Critical caching configuration issues found", issues=critical_issues
            )
