...
class SecretValidationService:
    ...
    def _initialize_validation_rules(self) -> dict[str, ValidationRule]:
        ...
        rules['no_weak_patterns'] = ValidationRule(
            ...
            custom_params={
                'weak_patterns': [
                    'password', '123456', 'admin', 'default', 'secret',
    ...
    def _initialize_compliance_mappings(self) -> dict[ComplianceStandard, dict[str, Any]]:
        ...
        return {
            ComplianceStandard.NIST_800_53: {
                ...
                'required_rules': [
                    'minimum_length', 'entropy_threshold', 'no_weak_patterns',
                    'character_diversity', 'rotation_age_check', 'encryption_compliance'
                ]
            },
            ComplianceStandard.ISO_27001: {
                ...
                'required_rules': [
                    'minimum_length', 'entropy_threshold', 'character_diversity',
                    'rotation_age_check', 'access_frequency_check'
                ]
            },
            ComplianceStandard.SOC2_TYPE2: {
                'name': 'SOC 2 Type II',
                ...
                'required_rules': [
                    'minimum_length', 'rotation_age_check', 'access_frequency_check',
                    'exposure_check'
                ]
            },
            ComplianceStandard.PCI_DSS: {
                ...
                'required_rules': [
                    'entropy_threshold', 'encryption_compliance', 'rotation_age_check'
                ]
            }
        }
    ...
    async def _execute_validation_rule(
        ...
        try:
            if rule.name == "minimum_length":
                return self._validate_minimum_length(rule, secret_name, secret_value, environment)
            elif rule.name == "entropy_threshold":
                return self._validate_entropy_threshold(rule, secret_name, secret_value, environment)
            elif rule.name == "no_weak_patterns":
                return self._validate_no_weak_patterns(rule, secret_name, secret_value, environment)
            elif rule.name == "character_diversity":
                return self._validate_character_diversity(rule, secret_name, secret_value, environment)
            elif rule.name == "no_dictionary_words":
                return self._validate_no_dictionary_words(rule, secret_name, secret_value, environment)
            elif rule.name == "rotation_age_check":
                return await self._validate_rotation_age_check(rule, secret_name, secret_value, environment)
            elif rule.name == "access_frequency_check":
                return await self._validate_access_frequency_check(rule, secret_name, secret_value, environment)
            elif rule.name == "encryption_compliance":
                return self._validate_encryption_compliance(rule, secret_name, secret_value, environment)
            elif rule.name == "exposure_check":
                return await self._validate_exposure_check(rule, secret_name, secret_value, environment)
            else:
    ...
    def _validate_entropy_threshold(
        ...
        return ValidationResult(
            ...
            details={
                ...
                'entropy_per_character': actual_entropy / len(secret_value) if secret_value else 0
            },
    ...
    def _validate_no_dictionary_words(
        ...
        common_words = [
            'password', 'admin', 'user', 'login', 'secret', 'key', 'token',
    ...
    async def _validate_exposure_check(
        ...
        return ValidationResult(
            ...
            details={
                ...
                'checks_performed': [
                    key for key, enabled in rule.custom_params.items()
