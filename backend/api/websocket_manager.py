...
class WebSocketManager:
        ...
        for client_id in self.rooms[room_name]:
            if client_id in self.active_connections:
                try:
                    ...
                    logger.error(f"Failed to send to room {room_name}, client {client_id}: {e}")
                    disconnected.append(client_id)
    ...
    async def send_rag_response(self, client_id: str, query: str, response: str, document_id: int):
    ...
    async def send_index_progress(self, client_id: str, document_id: int, status: str, progress: int = None):
    ...
    async def send_document_update(self, document_id: int, action: str, data: dict = None):
