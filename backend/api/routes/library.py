...
async def optimize_library(
    ...
    try:
        ...
        if results.get("orphaned_indexes_cleaned", 0) > 0:
            optimizations.append(f"Removed {results['orphaned_indexes_cleaned']} orphaned indexes")
        
        if results.get("invalid_indexes_cleaned", 0) > 0:
            optimizations.append(f"Removed {results['invalid_indexes_cleaned']} invalid indexes")
        
        if results.get("cache_optimized", 0) > 0:
            optimizations.append(f"Optimized {results['cache_optimized']} cache entries")
        
        if results.get("storage_optimized", 0) > 0:
            optimizations.append(f"Optimized {results['storage_optimized']} storage items")
