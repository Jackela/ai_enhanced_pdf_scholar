...
class UploadSession(BaseModel):
    ...
    chunk_size: int = Field(default=8388608, gt=0, le=16777216, description="Chunk size (8MB default, 16MB max)")
    total_chunks: int = Field(..., gt=0, description="Total number of chunks")
    uploaded_chunks: int = Field(default=0, ge=0, description="Number of uploaded chunks")
    ...
    def validate_total_chunks(cls, v: int, info) -> int:
        ...
        if hasattr(info, 'data'):
            ...
            if total_size and chunk_size:
                ...
                if v != expected:
                    raise ValueError(f"Invalid total_chunks: expected {expected}, got {v}")
        return v
