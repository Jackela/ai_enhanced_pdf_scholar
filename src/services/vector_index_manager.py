...
class VectorIndexManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"Vector index manager initialized with storage: {storage_base_dir}")
    
    def create_index_storage(self, 
                           document_id: int, 
                           index_hash: str,
                           chunk_count: int = 0) -> VectorIndexModel:
        """
        ...
        try:
            ...
            if index_path.exists():
                logger.warning(f"Index path already exists, adding suffix: {index_path}")
            ...
            logger.error(f"Failed to create index storage for document {document_id}: {e}")
            raise VectorIndexManagerError(f"Index storage creation failed: {e}") from e
    
    def move_index_to_storage(self, 
                            vector_index: VectorIndexModel,
        ...
        try:
            ...
            if not source_path.exists():
                raise VectorIndexManagerError(f"Source path does not exist: {source_path}")
            ...
            if missing_files:
                raise VectorIndexManagerError(f"Missing required files: {missing_files}")
            ...
            if not self.verify_index_integrity(vector_index.id):
                raise VectorIndexManagerError("Index integrity verification failed after move")
    ...
    def verify_index_integrity(self, vector_index_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Index integrity check completed for {vector_index_id}: {'VALID' if result['is_valid'] else 'INVALID'}")
            return result
    ...
    def backup_index(self, vector_index_id: int) -> str:
        ...
        try:
            vector_index = self.vector_repo.find_by_id(vector_index_id)
            if not vector_index:
                raise VectorIndexManagerError(f"Vector index not found: {vector_index_id}")
            
            source_path = Path(vector_index.index_path)
            if not source_path.exists():
                raise VectorIndexManagerError(f"Index path does not exist: {source_path}")
    ...
    def get_storage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if index_sizes:
                stats["average_index_size_mb"] = round(sum(index_sizes) / len(index_sizes) / (1024 * 1024), 2)
    ...
    def _extract_chunk_count(self, index_path: Path) -> int:
        ...
        try:
            ...
            logger.warning(f"Could not extract chunk count from {index_path}: {e}")
            return 0
    ...
    def _remove_corrupted_indexes(self) -> int:
        ...
        try:
            ...
            for vector_index in self.vector_repo.find_all():
                ...
                if not integrity["is_valid"] and "does not exist" not in str(integrity.get("errors", [])):
                    ...
                    try:
                        ...
                        logger.warning(f"Could not backup corrupted index {vector_index.id}: {e}")
