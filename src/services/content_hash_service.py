...
class ContentHashService:
    ...
    def calculate_combined_hashes(file_path: str) -> Tuple[str, str]:
        ...
        try:
            ...
            logger.debug(f"Calculated combined hashes for {file_path}: file={file_hash}, content={content_hash}")
        ...
        try:
            ...
            if info["is_pdf"] and path.exists():
                try:
                    ...
                    info["is_valid_pdf"] = ContentHashService.validate_pdf_file(file_path)
                    ...
                    logger.debug(f"Could not get detailed PDF info for {file_path}: {e}")
