...
class LargeDocumentProcessor:
    ...
    def process_document(self, file_path: str, content: Optional[str] = None) -> list[dict[str, Any]]:
        """
        ...
        """
        op_id = self.monitor.start_operation("process_large_document", {"file": file_path})
        ...
        try:
            ...
            if size_mb > self.config.max_file_size_mb:
                raise ValueError(f"Document size ({size_mb:.2f}MB) exceeds maximum ({self.config.max_file_size_mb}MB)")
    ...
    def _get_document_hash(self, file_path: str, content: Optional[str] = None) -> str:
    ...
    def _load_document(self, file_path: str) -> str:
        ...
        if size_mb > self.config.max_file_size_mb:
            raise ValueError(f"File size ({size_mb:.2f}MB) exceeds maximum ({self.config.max_file_size_mb}MB)")
    ...
    def process_batch(self, file_paths: list[str]) -> dict[str, list[dict[str, Any]]]:
        ...
        """
        op_id = self.monitor.start_operation("process_batch", {"count": len(file_paths)})
        results = {}
