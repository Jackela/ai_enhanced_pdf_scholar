...
class RAGFileManager:
    ...
    def verify_index_files(self, index_path: str) -> bool:
        ...
        try:
            ...
            if not path.exists() or not path.is_dir():
                logger.debug(f"Index path does not exist or is not directory: {index_path}")
                return False
            ...
            for file_name in required_files:
                ...
                if file_name.endswith('.json'):
                    try:
                        ...
                        logger.debug(f"JSON file validation failed for {file_name}: {e}")
                        return False
    ...
    def copy_index_files(self, source_path: Path, dest_path: Path) -> None:
        ...
        try:
            ...
            logger.debug(f"Index files copied successfully: {source_path} → {dest_path}")
            
        except Exception as e:
            error_msg = f"Failed to copy index files from {source_path} to {dest_path}: {e}"
            logger.error(error_msg)
            raise RAGFileManagerError(error_msg) from e

    def prepare_index_directory(self, index_path: Path, overwrite: bool = False) -> None:
        """
        ...
        try:
            if index_path.exists():
                if overwrite:
                    ...
                    raise RAGFileManagerError(f"Index directory already exists: {index_path}")
    ...
    def cleanup_index_files(self, index_path: str) -> bool:
        ...
        try:
            ...
            if path.exists():
                ...
                logger.debug(f"Index path does not exist, nothing to cleanup: {index_path}")
                return True
        ...
        try:
            ...
            if vector_store_path.exists():
                with open(vector_store_path) as f:
                    ...
                    logger.debug(f"Chunk count estimated from vector store: {chunk_count}")
                    return chunk_count
    ...
    def cleanup_orphaned_directories(self, orphaned_dirs: List[Path]) -> int:
        ...
        for orphaned_dir in orphaned_dirs:
            try:
                ...
                logger.warning(f"Failed to remove orphaned directory {orphaned_dir}: {e}")
    ...
    def get_storage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            if self.vector_storage_dir.exists():
                ...
                index_dirs = [d for d in self.vector_storage_dir.iterdir() if d.is_dir()]
    ...
    def is_accessible(self) -> bool:
        ...
        try:
            return (
                ...
                self.vector_storage_dir.stat().st_mode  # Check permissions
            )
        except Exception as e:
