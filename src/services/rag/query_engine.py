...
class RAGQueryEngine:
    ...
    def load_index_for_document(self, document_id: int) -> bool:
        ...
        try:
            ...
            if not self.file_manager.verify_index_files(vector_index.index_path):
                raise IndexLoadError(
                    f"Vector index files missing or corrupted: {vector_index.index_path}"
                )
            ...
            error_msg = (
                f"Unexpected error loading index for document {document_id}: {e}"
            )
    ...
    def _load_vector_index(self, vector_index: VectorIndexModel) -> "VectorStoreIndex":
        ...
        try:
            ...
            error_msg = (
                f"Failed to load vector index from {vector_index.index_path}: {e}"
            )
            logger.error(error_msg)
            raise IndexLoadError(error_msg) from e
    ...
    def _create_mock_index(self, document_id: int) -> Any:
        ...
        class MockQueryEngine:
            ...
            def query(self, query_text: str) -> "MockResponse":
                return MockResponse(
                    f"Mock response for document {self.doc_id}: {query_text}"
                )
    ...
    def query_document(self, query: str, document_id: int) -> str:
        ...
        try:
            ...
            if self.current_document_id != document_id or not self.current_index:
                try:
                    ...
                    raise QueryExecutionError(
                        f"Failed to load index for query: {e}"
                    ) from e
            ...
            logger.info(
                f"Query completed for document {document_id} in {query_duration.total_seconds():.2f}s"
            )
    ...
    def get_document_query_status(self, document_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            if vector_index:
                ...
                status["created_at"] = (
                    vector_index.created_at.isoformat()
                    if vector_index.created_at
                    else None
                )

                # Verify index files
                status["index_valid"] = self.file_manager.verify_index_files(
