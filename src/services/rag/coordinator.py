...
from .recovery_service import RAGRecoveryError, RAGRecoveryService
...
class RAGCoordinator:
    ...
    def build_index_from_document(
        ...
        logger.info(
            f"Coordinating index build for document {document.id}: {document.title}"
        )
        ...
        try:
            ...
            build_result = self.index_builder.build_index_for_document(
                document, overwrite
            )
        ...
        try:
            health_status = {
                ...
                "services": {
                    ...
                    "database": {"healthy": True, "connection_valid": True},
                    "recovery_service": {"healthy": True, "health_checks": {}},
                },
                "recommendations": [],
            }
            ...
            try:
                ...
                if not all(recovery_health.values()):
                    ...
                    health_status["overall_healthy"] = False
                    failed_checks = [
                        check for check, result in recovery_health.items() if not result
                    ]
                    health_status["recommendations"].extend(
                        [
                            f"Address failed health check: {check}"
