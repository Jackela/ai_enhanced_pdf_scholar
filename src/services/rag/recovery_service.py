...
class RAGRecoveryService:
    ...
    def _setup_health_checks(self) -> None:
        ...
        self.health_checker.add_health_check("vector_storage", self._check_vector_storage_health)
        self.health_checker.add_health_check("database_connection", self._check_database_health)
        self.health_checker.add_health_check("system_resources", self._check_system_resources)
    ...
    def _check_system_resources(self) -> bool:
        ...
        try:
            ...
            return (
                memory.percent < 90.0 and  # Less than 90% memory usage
                disk_usage.free > 1024 * 1024 * 1024  # At least 1GB free space
            )
        except Exception as e:
    ...
    def analyze_index_corruption(self, vector_index: VectorIndexModel) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Corruption analysis completed: {analysis_result['corruption_severity']}")
            return analysis_result
    ...
    def _analyze_file_integrity(self, file_path: Path, analysis_result: Dict[str, Any]) -> None:
                ...
                try:
                    ...
                    if file_path.name == "default__vector_store.json":
                        if not isinstance(data, dict) or "embedding_dict" not in data:
                            analysis_result["corrupted_files"].append(f"{file_path.name}: Invalid structure")
                            analysis_result["corruption_detected"] = True
                            analysis_result["corruption_types"].append("corrupted_files")
                            
                except json.JSONDecodeError as e:
                    analysis_result["corrupted_files"].append(f"{file_path.name}: JSON decode error - {e}")
                    analysis_result["corruption_detected"] = True
                    analysis_result["corruption_types"].append("corrupted_files")
                    
        except Exception as e:
            analysis_result["corrupted_files"].append(f"{file_path.name}: Analysis error - {e}")
            analysis_result["corruption_detected"] = True
    ...
    def _determine_corruption_severity(self, analysis_result: Dict[str, Any]) -> None:
        ...
        if (analysis_result["missing_files"] or 
            any("vector_store" in f for f in analysis_result["corrupted_files"]) or
            "missing_directory" in analysis_result["corruption_types"]):
            analysis_result["corruption_severity"] = "critical"
            
        # Moderate: Corrupted non-core files or empty files
        elif (analysis_result["corrupted_files"] or 
              analysis_result["file_size_issues"]):
    ...
    def _generate_recovery_recommendations(self, analysis_result: Dict[str, Any]) -> None:
    ...
    def recover_corrupted_index(
        ...
        try:
            ...
            if not corruption_analysis["corruption_detected"] and not force_rebuild:
                ...
                if force_rebuild or corruption_analysis["corruption_severity"] == "critical":
                    ...
                    if rebuild_callback:
                        ...
                        recovery_result["error"] = "Rebuild required but no callback provided"
                    ...
                    if not success and rebuild_callback:
                        ...
                        recovery_result["recovery_successful"] = rebuild_callback(vector_index)
            ...
            recovery_result["recovery_duration_ms"] = int(recovery_duration.total_seconds() * 1000)
        ...
        try:
            ...
            if vector_store_file.exists() and vector_store_file.stat().st_size > 0:
                try:
                    ...
                    if isinstance(vector_data, dict) and "embedding_dict" in vector_data:
    ...
    def _regenerate_index_metadata(self, index_path: Path, vector_data: Dict[str, Any]) -> None:
    ...
    def _perform_verification_repair(self, vector_index: VectorIndexModel) -> bool:
        ...
        try:
            ...
            if self.file_manager.verify_index_files(vector_index.index_path):
                ...
                actual_chunk_count = self.file_manager.get_chunk_count(vector_index.index_path)
                
                if actual_chunk_count != vector_index.chunk_count and actual_chunk_count > 0:
                    vector_index.chunk_count = actual_chunk_count
                    self.vector_repo.update(vector_index)
                    logger.info(f"Updated chunk count for index {vector_index.id}: {actual_chunk_count}")
    ...
    def perform_system_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if corrupted_indexes:
                health_report["overall_status"] = "degraded" if health_report["overall_status"] == "healthy" else "critical"
                health_report["recommendations"].append(f"Repair or rebuild {len(corrupted_indexes)} corrupted indexes")
                
            # Check for orphaned resources
            orphaned_count = self.cleanup_orphaned_resources()
            if orphaned_count > 0:
                health_report["cleanup_actions"].append(f"Cleaned up {orphaned_count} orphaned resources")
            ...
            health_report["check_duration_ms"] = int(check_duration.total_seconds() * 1000)
                ...
                try:
                    ...
                    if corruption_analysis["corruption_detected"]:
                        corrupted_info = {
                            "index_id": vector_index.id,
                            "document_id": vector_index.document_id,
                            "corruption_severity": corruption_analysis["corruption_severity"],
    ...
    def cleanup_orphaned_resources(self) -> int:
        ...
        try:
            ...
            if total_orphans > 0:
                logger.info(f"Cleaned up {total_orphans} orphaned resources ({db_orphans} DB + {fs_orphans} FS)")
