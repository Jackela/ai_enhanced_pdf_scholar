...
class DocumentProcessingError(RAGProcessingError):
    ...
    def __init__(self, document_id: int, stage: str, message: str, **kwargs):
        ...
        suggestions = [
            ...
            f"Check document format and integrity",
...
class QueryProcessingError(RAGQueryError):
    ...
    def __init__(self, query: str, document_id: Optional[int] = None, message: str = "", **kwargs):
        self.query = query
        self.document_id = document_id
        details = {
            "query": query,
        ...
        super().__init__(message or f"Failed to process query: {query[:100]}...", 
...
class StorageQuotaExceededError(RAGStorageError):
    ...
    def __init__(self, current_usage: int, quota_limit: int, **kwargs):
        ...
        message = f"Storage quota exceeded: {current_usage}MB used of {quota_limit}MB limit"
        super().__init__(message, details=details, suggestions=suggestions)
...
class MemoryLimitExceededError(RAGMemoryError):
    ...
    def __init__(self, current_usage: int, memory_limit: int, **kwargs):
        ...
        message = f"Memory limit exceeded: {current_usage}MB used of {memory_limit}MB limit"
        super().__init__(message, details=details, suggestions=suggestions)
...
def create_processing_error(document_id: int, stage: str, original_exception: Exception) -> DocumentProcessingError:
...
def create_query_error(query: str, document_id: Optional[int], original_exception: Exception) -> QueryProcessingError:
