...
class EnhancedRAGService:
    ...
    def __init__(self, 
        ...
        logger.info(f"Enhanced RAG service initialized with vector storage: {vector_storage_dir}")
    ...
    def build_index_from_pdf(self, pdf_path: str, cache_dir: str = None) -> bool:
        """
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.vector_storage_dir.exists():
                vector_dirs = [d for d in self.vector_storage_dir.iterdir() if d.is_dir()]
    ...
    def build_index_from_document(self, document: DocumentModel, overwrite: bool = False) -> VectorIndexModel:
        """
        ...
        if existing_index and not overwrite:
            raise RAGIndexError(f"Vector index already exists for document {document.id}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            vector_index_path = self.vector_storage_dir / f"doc_{document.id}_{index_hash[:8]}"
            ...
            chunk_count = self._get_chunk_count(vector_index_path)
            ...
            logger.info(f"Vector index for document {document.id} built successfully: {saved_index.id}")
            return saved_index
    ...
    def load_index_for_document(self, document_id: int) -> bool:
        ...
        if not vector_index:
            raise VectorIndexNotFoundError(f"No vector index found for document {document_id}")

        # Verify index files exist
        if not self._verify_index_files(vector_index.index_path):
            raise RAGIndexError(f"Vector index files missing: {vector_index.index_path}")

        # Load index using base RAG service
        if not self.test_mode:
            from llama_index.core import StorageContext, load_index_from_storage

            storage_context = StorageContext.from_defaults(persist_dir=vector_index.index_path)
    ...
    def get_document_index_status(self, document_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if vector_index:
                ...
                # Verify index files
                status["index_valid"] = self._verify_index_files(vector_index.index_path)
    ...
    def _load_existing_index(self, document: DocumentModel, vector_index: VectorIndexModel) -> VectorIndexModel:
            ...
            if not self.test_mode:
                ...
                self.current_index = type('MockIndex', (), {
                    'as_query_engine': lambda **kwargs: type('MockQueryEngine', (), {
                        'query': lambda prompt: type('MockResponse', (), {
    ...
    def _create_mock_index(self, document_id: int):
        """
        ...
        try:
            ...
            if metadata_path.exists():
                ...
                with open(metadata_path, 'r') as f:
                    ...
                    return metadata.get("document_count", 0)
