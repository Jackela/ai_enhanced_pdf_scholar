...
class RAGCacheService:
    ...
    def __init__(self, 
        ...
        logger.info(f"RAG cache service initialized: max_entries={max_entries}, ttl={ttl_hours}h")
    ...
    def _initialize_cache_table(self):
        ...
        try:
            ...
            self.db.execute("CREATE INDEX IF NOT EXISTS idx_cache_document ON rag_query_cache(document_id)")
            self.db.execute("CREATE INDEX IF NOT EXISTS idx_cache_accessed ON rag_query_cache(accessed_at DESC)")
            self.db.execute("CREATE INDEX IF NOT EXISTS idx_cache_hash ON rag_query_cache(query_hash)")
    ...
    def cache_response(self, 
        ...
        try:
            ...
            logger.debug(f"Cached query response: hash={query_hash[:8]}, doc={document_id}")
            return True
    ...
    def invalidate_document_cache(self, document_id: int) -> int:
        ...
        try:
            result = self.db.execute(
                "DELETE FROM rag_query_cache WHERE document_id = ?",
                (document_id,)
            )
            
            removed_count = result.rowcount if hasattr(result, 'rowcount') else 0
            logger.info(f"Invalidated {removed_count} cache entries for document {document_id}")
    ...
    def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            hit_rate = (self.metrics["cache_hits"] / total_queries * 100) if total_queries > 0 else 0
            ...
            stats = {
                ...
                "total_storage_kb": round((cache_stats["total_query_length"] + cache_stats["total_response_length"]) / 1024, 2) if cache_stats else 0,
    ...
    def _get_exact_match(self, query_hash: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return self.db.fetch_one(
                "SELECT * FROM rag_query_cache WHERE query_hash = ?",
                (query_hash,)
            )
        except Exception:
            return None
    
    def _find_similar_query(self, query: str, document_id: int) -> Optional[Dict[str, Any]]:
        """
    ...
    def _enforce_cache_size(self) -> int:
        ...
        try:
            ...
            current_count = self.db.fetch_one("SELECT COUNT(*) as count FROM rag_query_cache")["count"]
            
            if current_count <= self.max_entries:
                return 0
            
            # Calculate how many to remove
            to_remove = current_count - self.max_entries + 100  # Remove extra for buffer
            ...
            for entry in lru_entries:
                self.db.execute("DELETE FROM rag_query_cache WHERE id = ?", (entry["id"],))
                removed_count += 1
    ...
    def _clean_expired_entries(self) -> int:
        ...
        try:
            ...
            result = self.db.execute(
                "DELETE FROM rag_query_cache WHERE created_at < ?",
                (cutoff_str,)
            )
