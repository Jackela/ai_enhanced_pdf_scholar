...
class ConnectionPool:
    ...
    def __init__(self, db_path: str, max_connections: int = 20, connection_timeout: float = 30.0):
        ...
        logger.info(f"Connection pool initialized: {self.db_path} (max: {max_connections})")
    ...
    def _create_connection(self) -> sqlite3.Connection:
        ...
        try:
            ...
            conn.execute("PRAGMA page_size = 65536")  # 64KB page size for better performance
            ...
            logger.debug(f"Created optimized database connection (total: {self._stats['created']})")
            return conn
...
class DatabaseConnection:
    ...
    def _get_current_connection(self) -> ConnectionInfo:
        ...
        if not hasattr(self._local, 'connection_info') or self._local.connection_info is None:
        ...
        return self._local.connection_info
    ...
    def transaction(self, savepoint_name: str = None):
        ...
        savepoint_name = savepoint_name or f"sp_{conn_info.transaction_level}_{int(time.time() * 1000)}"
        ...
        if is_nested:
            ...
            try:
                ...
                logger.debug(f"Started savepoint: {savepoint_name} (level: {conn_info.transaction_level})")
                
                yield conn
                
                conn.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                conn_info.transaction_level -= 1
                logger.debug(f"Released savepoint: {savepoint_name} (level: {conn_info.transaction_level})")
                
            except Exception as e:
                try:
                    conn.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    conn_info.transaction_level -= 1
                    logger.debug(f"Rolled back to savepoint: {savepoint_name} (level: {conn_info.transaction_level})")
                except Exception as rollback_error:
                    logger.error(f"Failed to rollback to savepoint {savepoint_name}: {rollback_error}")
                
                logger.error(f"Transaction rolled back to savepoint {savepoint_name}: {e}")
    ...
    def execute(self, query: str, params: tuple = None, max_retries: int = 3) -> sqlite3.Cursor:
        """
            ...
            try:
                ...
                if "database is locked" in error_msg or "database is busy" in error_msg:
                    if attempt < max_retries:
                        ...
                        logger.warning(f"Database busy, retrying in {wait_time}s (attempt {attempt + 1}/{max_retries + 1})")
