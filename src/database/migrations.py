...
class DatabaseMigrator:
    ...
    def _migration_004_performance_optimization(self) -> None:
        ...
        for i, index_sql in enumerate(all_indexes, 1):
            try:
                self.db.execute(index_sql)
                logger.debug(f"Created index {i}/{len(all_indexes)}: {index_sql[:50]}...")
            except Exception as e:
    ...
    def _migration_005_advanced_performance_analysis(self) -> None:
        ...
        """
        logger.info("Applying migration 005: Advanced performance analysis and monitoring")
        ...
        logger.info(f"Creating {len(all_advanced_indexes)} advanced performance indexes...")
        ...
        for i, index_sql in enumerate(all_advanced_indexes, 1):
            try:
                self.db.execute(index_sql)
                created_count += 1
                logger.debug(f"Created advanced index {i}/{len(all_advanced_indexes)}: {index_sql[:60]}...")
            except Exception as e:
                logger.warning(f"Advanced index creation warning for index {i}: {e}")
                # Continue with other indexes even if one fails

        logger.info(f"Successfully created {created_count}/{len(all_advanced_indexes)} advanced indexes")
        ...
        try:
            ...
            try:
                ...
                if index_result:
                    ...
                    baselines.append(("total_indexes", index_count, "count", "Total user-defined indexes"))
            except Exception as e:
                logger.warning(f"Could not get index count: {e}")
            
            # Database file size
            try:
                size_result = self.db.fetch_one("SELECT page_count * page_size as size FROM pragma_page_count(), pragma_page_size()")
                if size_result and size_result['size']:
                    db_size = size_result['size']
                    baselines.append(("database_size_bytes", db_size, "bytes", "Database file size"))
            except Exception as e:
            ...
            sample_queries = [
                ...
                ("SELECT d.*, COUNT(c.id) FROM documents d LEFT JOIN citations c ON d.id = c.document_id GROUP BY d.id LIMIT 10", "document_citation_join"),
            ]
        ...
        try:
            ...
            for table_row in tables:
                ...
                try:
                    ...
                    table_size = dbstat_result["size"] if dbstat_result and dbstat_result["size"] else 0
                    
                    table_stats.append({
                        "table_name": table_name,
                        "row_count": row_count,
                        "estimated_size_bytes": table_size,
                        "avg_row_size_bytes": table_size / row_count if row_count > 0 else 0
                    })
        ...
        try:
            ...
            for index_row in indexes:
                ...
                try:
                    ...
                    logger.warning(f"Could not get statistics for index {index_name}: {e}")
    ...
    def _get_query_performance_hints(self) -> dict[str, Any]:
        ...
        try:
            ...
            common_queries = [
                ("documents", "title", "Text search on title without LIKE optimization"),
    ...
    def _get_maintenance_recommendations(self) -> list[str]:
        ...
        try:
            ...
            recommendations.append("Run ANALYZE periodically to update query optimizer statistics")
            
            # Check journal mode
            journal_mode = self.db.fetch_one("PRAGMA journal_mode")
            if journal_mode and journal_mode[0] != "WAL":
                recommendations.append("Consider using WAL mode (PRAGMA journal_mode=WAL) for better concurrent access")
            
            # Check cache size
            cache_size = self.db.fetch_one("PRAGMA cache_size")
            if cache_size and abs(cache_size[0]) < 2000:  # Less than 2MB cache
                recommendations.append("Consider increasing cache size (PRAGMA cache_size) for better performance")
                
            # Check for unused indexes
            recommendations.append("Monitor index usage and remove unused indexes to save space")
            
            # Vacuum recommendation
            recommendations.append("Run VACUUM periodically to reclaim unused space and optimize file layout")
    ...
    def get_advanced_query_analysis(self, query: str, params: tuple = ()) -> dict[str, Any]:
        """
        ...
        try:
            ...
            plan_text = " ".join([step.get("detail", "") for step in analysis["execution_plan"]])
            analysis["index_usage"] = self._extract_index_usage_from_plan(plan_text)
            
            # Generate optimization recommendations
            analysis["optimization_recommendations"] = self._generate_query_optimization_recommendations(
    ...
    def _assess_query_complexity(self, query: str) -> str:
        ...
        if complexity_indicators["complex"] > 0:
            return "complex"
        elif complexity_indicators["moderate"] > 2:
            return "complex"
    ...
    def _generate_query_optimization_recommendations(self, query: str, execution_plan: list, execution_time: float) -> list[str]:
        ...
        if "scan table" in plan_text.lower():
            recommendations.append("Table scan detected. Consider adding appropriate indexes.")
        
        # LIKE pattern optimization
        if "like" in query_lower and "%" in query:
            if query.count("%") >= 2:  # Leading and trailing wildcards
                recommendations.append("LIKE with leading wildcards can't use indexes efficiently. Consider FTS if full-text search is needed.")
        ...
        if "group by" in query_lower:
            recommendations.append("GROUP BY queries can benefit from indexes on grouping columns.")
        
        # Large LIMIT without index
        if "limit" in query_lower and "using index" not in plan_text.lower():
            recommendations.append("LIMIT queries should use indexes for optimal performance.")
        
        # JOIN optimization
        if "join" in query_lower and "nested loop" in plan_text.lower():
            recommendations.append("JOIN operations should have indexes on join columns for better performance.")
            
        return recommendations

    def _analyze_query_cost(self, query: str, execution_plan: list, rows_returned: int) -> dict[str, Any]:
        ...
        if "scan table" in plan_text.lower():
            cost_analysis["scalability_concerns"].append("Full table scans don't scale with data size")
        if rows_returned > 10000:
            cost_analysis["scalability_concerns"].append("Large result sets may cause memory issues")
        
        return cost_analysis

    def benchmark_query_performance(self, queries: list[tuple[str, str, tuple]]) -> dict[str, Any]:
        """
        ...
        try:
            ...
            for query_name, query_sql, params in queries:
                ...
                if execution_times:
                    ...
                    benchmark_result = {
                        ...
                        "consistency": "high" if (max_time - min_time) < avg_time * 0.2 else "low"
                    }
            ...
            if benchmark_results["benchmarks"]:
                ...
                fastest = min(benchmark_results["benchmarks"], key=lambda x: x["average_time_ms"])
                slowest = max(benchmark_results["benchmarks"], key=lambda x: x["average_time_ms"])
                ...
                benchmark_results["performance_summary"] = {
                    ...
                    "performance_variance": slowest["average_time_ms"] / fastest["average_time_ms"] if fastest["average_time_ms"] > 0 else 0
                }
    ...
    def analyze_index_effectiveness(self) -> dict[str, Any]:
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        if "UNIQUE" in (index_row["sql"] or "").upper():
                            ...
                            index_analysis["selectivity_estimate"] = 0.5  # Conservative estimate
            ...
            for index_info in analysis["indexes"]:
                ...
                elif index_info["effectiveness"] == "unknown" and index_info["size_estimate"] > 0:
            ...
            analysis["summary"] = {
                ...
                "index_effectiveness_ratio": len(analysis["high_impact"]) / analysis["total_indexes"] if analysis["total_indexes"] > 0 else 0
            }
    ...
    def _migration_006_add_authentication_tables(self) -> None:
        ...
        try:
            self.db.execute(admin_user_sql, (
                "admin",
                "admin@localhost",
                password_hash,
