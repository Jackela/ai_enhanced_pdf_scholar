...
class DatabaseMigrator:
    ...
    def _migration_002_add_content_hash(self):
        """
        ...
        create_index_sql = "CREATE INDEX idx_documents_content_hash ON documents(content_hash)"
    ...
    def get_schema_info(self) -> Dict[str, any]:
        ...
        try:
            ...
            for table in tables:
                ...
                columns = [{"name": col["name"], "type": col["type"], "notnull": bool(col["notnull"])} 
    ...
    def validate_schema(self) -> bool:
        ...
        try:
            ...
            if current_version != self.CURRENT_VERSION:
                logger.warning(f"Schema version mismatch: {current_version} != {self.CURRENT_VERSION}")
                return False
            
            # Check required tables exist
            required_tables = ["documents", "vector_indexes", "tags", "document_tags"]
            existing_tables = [row["name"] for row in 
                             self.db.fetch_all("SELECT name FROM sqlite_master WHERE type='table'")]
