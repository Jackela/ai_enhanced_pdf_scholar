...
class MigrationRunner:
    ...
    def _migrate_up(self, target_version: int, result: dict, start_time: float) -> dict[str, Any]:
        """Handle upgrade migrations."""
        try:
            # Validate migration plan
            validation_issues = self.manager.validate_migration_sequence(target_version)
            if validation_issues:
                raise MigrationError(f"Migration validation failed: {validation_issues}")
                
            # Get migrations to apply
            pending_migrations = [
                v for v in self.manager.get_available_versions()
            ...
            logger.info(f"Applying {len(pending_migrations)} migrations: {pending_migrations}")
            
            # Apply each migration
            for i, version in enumerate(pending_migrations):
                self._report_progress(i, len(pending_migrations), f"Applying migration {version}")
                ...
                try:
                    ...
                    # Record failure
                    execution_time = (time.time() - migration_start) * 1000 if 'migration_start' in locals() else 0
                    self.version_tracker.record_migration_failed(
                        version, "apply", getattr(migration, 'description', f'Migration {version}'),
            ...
            self._report_progress(
                len(pending_migrations), len(pending_migrations),
    ...
    def _migrate_down(self, target_version: int, result: dict, start_time: float) -> dict[str, Any]:
        """Handle rollback migrations."""
        try:
            # Check if rollback is possible
            can_rollback, issues = self.manager.can_rollback_to(target_version)
            if not can_rollback:
                raise MigrationError(f"Cannot rollback to version {target_version}: {issues}")
            ...
            logger.info(f"Rolling back {len(rollback_sequence)} migrations: {rollback_sequence}")
            
            # Rollback each migration
            for i, version in enumerate(rollback_sequence):
                self._report_progress(i, len(rollback_sequence), f"Rolling back migration {version}")
                ...
                try:
                    ...
                    execution_time = (time.time() - migration_start) * 1000 if 'migration_start' in locals() else 0
                    self.version_tracker.record_migration_failed(
                        version, "rollback", getattr(migration, 'description', f'Migration {version}'),
                        execution_time, str(e)
                    )
            ...
            self._report_progress(
                len(rollback_sequence), len(rollback_sequence),
    ...
    def validate_schema(self) -> dict[str, Any]:
        ...
        try:
            ...
            required_tables = [
                "documents", "vector_indexes", "tags", "document_tags",
                "citations", "citation_relations", "users", "migration_versions"
            ]
