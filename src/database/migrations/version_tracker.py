...
class VersionTracker:
    ...
    def get_current_version(self) -> int:
        ...
        try:
            ...
            table_version = (
                ...
                if table_result and table_result["version"]
                else 0
            )
    ...
    def record_migration_rollback(
        self, version: int, description: str, execution_time_ms: float
    ) -> None:
        """
        ...
        try:
            ...
            self.db.execute(
                "DELETE FROM migration_versions WHERE version = ?", (version,)
            )
            ...
            new_version = (
                remaining_result["version"]
                if remaining_result and remaining_result["version"]
                else 0
            )
    ...
    def get_migration_info(self, version: int) -> dict[str, Any] | None:
        ...
        try:
            result = self.db.fetch_one(
                "SELECT * FROM migration_versions WHERE version = ?", (version,)
            )
