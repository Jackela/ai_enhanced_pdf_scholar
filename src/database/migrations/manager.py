...
class MigrationManager:
        ...
        try:
            ...
            try:
                    ...
                    if inspect.isclass(obj):
                        ...
                        try:
                            ...
                            if (
                                hasattr(obj, "__mro__")
                                and any(
                                    "BaseMigration" in str(cls) for cls in obj.__mro__
                                )
                                and obj.__name__ != "BaseMigration"
                            ):
                                migration_class = obj
                                break
                ...
                if migration_class is None:
                    logger.warning(
                        f"No migration class found in {filename}. Classes found: {classes_found}"
                    )
                    return
    ...
    def validate_migration_sequence(
        ...
        try:
            ...
            missing_versions = [
                v for v in expected_versions if v not in available_versions
            ]
            ...
            for version in rollback_sequence:
                try:
                    ...
                    if not migration.rollback_supported:
                        issues.append(f"Migration {version} does not support rollback")
                except Exception as e:
                    issues.append(
                        f"Cannot check rollback support for migration {version}: {e}"
                    )
    ...
    def get_migration_plan(self, target_version: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if target > current_version:
                # Upgrade plan
                pending = [
                    v for v in available_versions if current_version < v <= target
                ]
