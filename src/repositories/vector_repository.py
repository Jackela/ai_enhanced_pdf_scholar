...
class VectorIndexRepository(BaseRepository[VectorIndexModel]):
    ...
    def find_by_document_id(self, document_id: int) -> Optional[VectorIndexModel]:
        ...
        try:
            ...
            logger.error(
                f"Failed to find vector index by document ID {document_id}: {e}"
            )
            raise
    ...
    def delete_by_document_id(self, document_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete vector index for document {document_id}: {e}"
            )
            raise
        ...
        try:
            ...
            for index in invalid:
                try:
                    ...
                    logger.warning(
                        f"Failed to cleanup individual index {index.id}: {e}"
                    )
                    continue
    ...
    def get_index_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if coverage_result:
                coverage_dict = dict(coverage_result)
                stats["coverage"] = coverage_dict
                total_docs = coverage_dict.get("total_documents", 0)
                docs_with_index = coverage_dict.get("documents_with_index", 0)
                if total_docs > 0:
                    coverage_dict["coverage_percentage"] = (
                        docs_with_index / total_docs * 100
                    )
    ...
    def verify_index_integrity(self, index_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            doc_check = self.db.fetch_one(
                "SELECT id FROM documents WHERE id = ?", (index.document_id,)
            )
