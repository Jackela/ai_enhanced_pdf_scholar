...
class BaseRepository(ABC, Generic[T]):
    ...
    def update(self, model: T) -> T:
        ...
        try:
            ...
            if result.rowcount == 0:
                raise ValueError(f"No {self.get_table_name()} found with ID {entity_id}")
    ...
    def exists(self, id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to check existence of {self.get_table_name()} ID {id}: {e}")
            raise
    ...
    def find_by_field(self, field: str, value: Any) -> List[T]:
        ...
        try:
            ...
            logger.error(f"Failed to find {self.get_table_name()} by {field}={value}: {e}")
            raise
    
    def execute_custom_query(self, query: str, params: tuple = None) -> List[T]:
        """
