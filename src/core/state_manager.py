...
class StateManager:
    ...
    def update_state(self, path: str, update_func: Callable[[Any], Any], notify: bool = True) -> bool:
        """
    ...
    def subscribe(self, path: str, callback: Callable[[str, Any, Any, StateChangeType], None]) -> None:
        """
    ...
    def _set_nested_value(self, state_dict: Dict[str, Any], path: str, value: Any) -> None:
    ...
    def _notify_observers(self, path: str, new_value: Any, old_value: Any, change_type: StateChangeType) -> None:
        """
        ...
        if path in self._observers:
            for observer in self._observers[path]:
                try:
                    ...
                    logger.error(f"Error notifying observer {observer.__name__} for path '{path}': {e}")
                ...
                if re.match(pattern, path):
                    for observer in observers:
                        try:
                            ...
                            logger.error(f"Error notifying wildcard observer {observer.__name__}: {e}")
    ...
    def reset_state(self, section: str = None) -> None:
        ...
        if section:
            if section in self._state:
                ...
                self._notify_observers(section, self._state[section], old_state, StateChangeType.RESET)
                logger.info(f"Reset state section: {section}")
        else:
            old_state = self._state.copy()
            self._state = self._get_default_state()
            self._notify_observers('__all__', self._state, old_state, StateChangeType.RESET)
